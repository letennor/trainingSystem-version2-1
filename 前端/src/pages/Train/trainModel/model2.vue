        ...
        <el-row>
          <el-col :span="8"
            ...
            <div class="question-index">
              <el-button :type="index === Number($route.query.Q) ? 'primary' : ''"
                         circle
      ...
      <el-col :span="16">
        <div>
          ...
          <div class="subject-describe">
            ...
            <el-collapse>
              <el-collapse-item v-for="(questionIO, index) in questions[$route.query.Q]
                  .questionIOList"
                                :key="index"
                                :title="'示例' + (index + 1)">
            ...
            <codemirror v-model="userAnswer[$route.query.Q]"
                        :options="cmOption"
                        class="code-mirror"
...
export default {
  ...
  methods: {
      ...
      this.$confirm('确定要提交吗？确定后将进入判题页面且不能返回', '提示', {
        ...
        .then(() => {
          ...
          request({
            ...
            //将situation存在session里面
            for (let i = 0; i < res.data.complieSituation.length; i++) {
              this.complieSituation.push(res.data.complieSituation[i])
            }

            window.sessionStorage.setItem(
              'complieSituation',
              JSON.stringify(this.complieSituation)
            )
            // 跳转到判题页面
            // this.$router.push('/judge?J=0')
            this.$router.push({
              path: '/judge?',
              query: {
                J: 0,
                complieSituation: this.complieSituation,
              },
            })
          })
        })
    ...
    countDown() {
      ...
      this.value = 100 - (h * 3600 + m * 60 + s) * (100 / (this.limitTime * 60))
